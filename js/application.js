...
(function() {
  ...
  createNext = function() {
    ...
    for (ih = _i = 0; 0 <= height ? _i <= height : _i >= height; ih = 0 <= height ? ++_i : --_i) {
      newState[ih] = [];
      for (iw = _j = 0; 0 <= width ? _j <= width : _j >= width; iw = 0 <= width ? ++_j : --_j) {
        if (survives(state[ih][iw], getLiveNeighbors(ih, iw))) {
          newState[ih][iw] = 1;
        }
      }
    }
  ...
  survives = function(current, liveCount) {
    if (current === 1) {
      if (liveCount < 2) {
        return false;
      }
